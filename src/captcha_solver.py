...
class CaptchaSolver:
    ...
    # Default anchor parameters (can be overridden via __init__)
    DEFAULT_ANCHOR_V = "7gg7H51Q-naNfhmCP3_R47ho"
    DEFAULT_ANCHOR_MS = "20000"
    DEFAULT_EXECUTE_MS = "30000"
    DEFAULT_CB = "yv0004astgfg"
    ...
    def __init__(
        ...
        Args:
            base_url (str): The base URL for the captcha solver.
            verbose (bool, optional): Whether to print Captcha data information. Defaults to false.
            custom_sitekey (str, optional): Custom sitekey to use instead of extracting from page.
            custom_page_action (str, optional): Custom page action to use instead of extracting from page.
        """
        self._base_url = base_url
        self._verbose = verbose
        self.sitekey = custom_sitekey
        self.page_action = custom_page_action
        # Anchor parameter configuration (use defaults when not provided)
        self._anchor_v = anchor_v or self.DEFAULT_ANCHOR_V
        self._anchor_ms = anchor_ms or self.DEFAULT_ANCHOR_MS
        self._execute_ms = execute_ms or self.DEFAULT_EXECUTE_MS
        self._cb = cb or self.DEFAULT_CB
    ...
    def _construct_url(self, port="443"):
        ...
        # Include path in constructed URL so 'co' encodes full resource (not only host)
        return f"{parsed_url.scheme}://{parsed_url.netloc}:{port}{path}"
    ...
    def _construct_anchor(self, sitekey, co_value, api_type):
        ...
        """
        # Add additional fixed parameters observed in the browser anchor URL.
        # Note: some of these (v, anchor-ms, execute-ms, cb) may change across requests;
        # keeping them configurable/dynamic is more robust, but fixed values help in testing.
        # Use configured anchor parameters (fall back to defaults set in __init__)
        return (
            f"{self.CAPTCHA_URL}/{api_type}/anchor?"
            f"ar=1&k={sitekey}&co={co_value}&hl=en&size=invisible"
            f"&v={self._anchor_v}&anchor-ms={self._anchor_ms}"
            f"&execute-ms={self._execute_ms}&cb={self._cb}"
        )
    ...
    def _get_anchor_token(self, client, anchor_url):
        ...
        """
        # Try several patterns to be robust against minor HTML differences.
        response = client.get(anchor_url)
        text = response.text
        # Common patterns observed in anchor HTML
        patterns = [
            r'recaptcha-token"\s*value=["\'](.*?)["\']',
            r'name=["\']recaptcha-token["\']\s*value=["\'](.*?)["\']',
            r'"recaptcha-token":\s*"?(03AFcWe[A-Za-z0-9_\-]+)"?',
            r'(03AFcWe[A-Za-z0-9_\-]{20,})',  # fallback: look for token-like string
        ]
        for pat in patterns:
            m = re.search(pat, text)
            if m:
                # return the first captured group that is non-empty
                for g in m.groups():
                    if g:
                        return g
                # if groups empty but match exists (fallback), return whole match
                return m.group(0)
        return None
    ...
    def solve_captcha(self, client):
        ...
        """
        if not self.sitekey:  # Only get sitekey if not already set (custom sitekey)
